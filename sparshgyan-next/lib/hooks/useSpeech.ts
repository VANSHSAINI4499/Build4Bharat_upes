...
export function useSpeech(options?: UseSpeechOptions) {
  ...
  const ttsRateRef = useRef(useAppStore.getState().ttsRate);
  ...
  // Keep refs in sync
  useEffect(() => {
    onVoiceCommandRef.current = options?.onVoiceCommand;
  }, [options?.onVoiceCommand]);
  useEffect(
    () =>
      useAppStore.subscribe((s) => {
        ttsEnabledRef.current = s.ttsEnabled;
        ttsRateRef.current = s.ttsRate;
  ...
  useEffect(() => {
    ...
    if (!SR) {
      console.warn(
        '[useSpeech] SpeechRecognition not supported — use Chrome or Edge',
      );
      useAppStore
        .getState()
        .setStatusMsg('Speech recognition not supported. Use Chrome or Edge.');
      useAppStore
        .getState()
        .setMicError(
    ...
    recognition.onresult = (event) => {
      console.log(
        '[useSpeech] onresult — resultIndex:',
        event.resultIndex,
        'results:',
      ...
      for (let i = event.resultIndex; i < event.results.length; i++) {
        ...
        if (event.results[i].isFinal) {
          ...
          if (
            autoVibrateRef.current &&
            arduinoConnectedRef.current &&
            enqueueRef.current
          ) {
          ...
          useAppStore
            .getState()
            .setWindowText(words.slice(-MAX_WORDS).join(' '));
    ...
    recognition.onerror = (event) => {
      ...
      useAppStore
        .getState()
        .setMicError(msgs[event.error] ?? `Recognition error: ${event.error}`);
  ...
  const startListening = useCallback(async () => {
    if (!recognitionRef.current) {
      useAppStore
        .getState()
        .setMicError(
          'Speech recognition not available. Please use Chrome or Edge.',
        );
      return;
    }
  ...
  const pauseListening = useCallback(() => {
    ...
    useAppStore.getState().setIsListening(false);
    useAppStore.getState().setStatusMsg('Paused');
  }, []);

  const resumeListening = useCallback(() => {
    window.speechSynthesis?.resume();
    if (recognitionRef.current && !useAppStore.getState().isListening) {
      shouldListenRef.current = true;
      try {
        recognitionRef.current.start();
      } catch {
        // Ignore start errors when recognition is already booting up.
      }
    }
    useAppStore.getState().setStatusMsg('Listening…');
  }, []);
  ...
  useEffect(() => {
    ...
    window.__sparshSpeechController = controller;

    return () => {
      if (window.__sparshSpeechController === controller) {
        delete window.__sparshSpeechController;
      }
    };
  }, [pauseListening, resumeListening, startListening, stopListening]);
